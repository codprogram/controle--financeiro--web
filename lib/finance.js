...
export const emptyIncomePlan = Object.freeze({
    setembro: "",
    outubro: "",
    novembro: "",
    dezembro: "",
...
export function normalizeIncomePlan(plan) {
    return Object.fromEntries(months.map((month) => [month.key, Number(plan?.[month.key] ?? 0)]));
}

export function incomePlanFormFromValues(plan) {
    return Object.fromEntries(months.map((month) => [month.key, String(plan?.[month.key] ?? "")]));
}

export function buildIncomePlanPayload(form) {
    return Object.fromEntries(
        months.map((month) => [month.key, Number(String(form?.[month.key] || "0").replace(",", ".")) || 0])
    );
}
